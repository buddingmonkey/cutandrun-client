...
updateGlobalBufferAndViews(buffer);
  ...
  HEAP32[DYNAMICTOP_PTR>>2] = 1815552;
...
// {{PRE_LIBRARY}}
  ...
  function _MultiplyNumbers(p1, p2)
      {
          //Greg: This is just proof that we can reference "ws" in this file at all. I think some sort of Setup WebSocket function will need to be created.
          ws.onmessage = message => {
              //message.data
              const response = JSON.parse(message.data);
          }
  
          console.log("multiply successfully called " + (p1)+" x "+(p2));
          return p1*p2;
      }

  function _SetUpTestCallback(pSimpleCallback) {
          console.log("[JavaScript] SetSimpleCallback");
  
          setTimeout(function () {
              dynCall_v(pSimpleCallback);// _v signature corresponds to `void ()`
          }, 0);
      }

  function _SetUpTimeCallback(pTimerCallback) {
  ...
  var _fetch_work_queue=1291248;function __emscripten_get_fetch_work_queue() {
  ...
  function _js_html_getRequiredMemoryCompressed(audioClipIdx)
  	{
          if (!this.audioContext || audioClipIdx < 0)
              return 0;
  ...
  function _js_html_getRequiredMemoryUncompressed(audioClipIdx)
  	{
          if (!this.audioContext || audioClipIdx < 0)
              return 0;
